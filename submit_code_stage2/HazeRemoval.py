# The code is taken from BLKStone's Github:
...
# Paper: http://kaiminghe.com/publications/pami10dehaze.pdf
import math, os
import numpy as np
import pandas as pd
import cv2
...
def img_dehaze(filePath):
	img = np.array(Image.open(filePath))
	dehazed_img = getRecoverScene(img, refine=True, omega=1)
	im = Image.fromarray(dehazed_img)
	if "train" in filePath:
		filePath = filePath.replace("train", "train_dehaze")
	elif "test" in filePath:
		filePath = filePath.replace("test", "test_dehaze")
	else:
...
def getRecoverScene(img, omega=0.95, t0=0.1, blockSize=15, meanMode=False, percent=0.001, refine=True):
	...
	if refine:        
		normI = (img - img.min()) / (img.max() - img.min() + 1e-10)  # normalize I
